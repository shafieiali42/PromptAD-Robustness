...
def disk(radius, alias_blur=0.1, dtype=np.float32):
    ...
    return cv.GaussianBlur(aliased_disk, ksize=ksize, sigmaX=alias_blur)
...
def defocus_blur(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        channels = np.array(cv.filter2D(x, -1, kernel))
    else:
        for d in range(3):
            channels.append(cv.filter2D(x[:, :, d], -1, kernel))
...
def snow(x, severity=1):
    ...
    if len(x.shape) < 3 or x.shape[2] < 3:
        ...
        x = c[6] * x + (1 - c[6]) * np.maximum(x, cv.cvtColor(x,
...
def spatter(x, severity=1):
    ...
    if c[5] == 0:
        ...
        if len(x.shape) < 3 or x.shape[2] < 3:
            ...
            x = cv.cvtColor(x, cv.COLOR_BGR2BGRA)

            return cv.cvtColor(np.clip(x + m * color, 0, 1),
                                cv.COLOR_BGRA2BGR) * 255
    else:
        m = np.where(liquid_layer > c[3], 1, 0)
        m = gaussian(m.astype(np.float32), sigma=c[4])
        m[m < 0.8] = 0

        x_rgb = np.array(Image.fromarray(x_PIL).convert('RGB'))
...
def pixelate(x, severity=1):
    ...
    x = Image.fromarray(x).resize((int(x_shape[1] * c), int(x_shape[0] * c)), Image.BOX)
